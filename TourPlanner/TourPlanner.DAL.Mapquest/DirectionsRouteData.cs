...
namespace TourPlanner.DAL.Mapquest
{
    public class DirectionsRouteData
    {
        ...
        public string getValuesBoundingBox()
        {
            //Example:
            //boundingBox = 40.06,-76.73,39.96,-76.30
            // latitude,longitude pair as an upper left corner bound and one for the lower right. It then creates a map based on those bounds given.
            string ul_lat = route.boundingBox.ul.lat.ToString().Replace(',', '.');
            string ul_lng = route.boundingBox.ul.lng.ToString().Replace(',', '.');
            string lr_lat = route.boundingBox.lr.lat.ToString().Replace(',', '.');
            string lr_lng = route.boundingBox.lr.lng.ToString().Replace(',', '.');
            string values = ul_lat + "," + ul_lng + "," + lr_lat + "," + lr_lng;
            return values;
        }

        public void PrintRouteInfo()
        {
            Console.WriteLine("tour distance: {0}", this.route.distance);
            Console.WriteLine("tour time: {0}", this.route.formattedTime);
        }
    }
